...
if( !function_exists( 'otw_pswl_init' ) ){
	...
	function otw_pswl_init(){
		...
		if( is_admin() ){
			...
			add_filter('otwfcr_notice', 'otw_pswl_factory_message' );
		}
		else
		{
		...
		$otw_pswl_factory_component = otw_load_component( 'otw_factory' );
		$otw_pswl_factory_object = otw_get_component( $otw_pswl_factory_component );
		$otw_pswl_factory_object->add_plugin( $otw_pswl_plugin_id, dirname( dirname( __FILE__ ) ).'/otw_popping_sidebars_and_widgets.php', array( 'menu_parent' => 'otw-pswl', 'lc_name' => __( 'License Manager', 'otw_pswl' ), 'menu_key' => 'otw-pswl' ) );
		
		include_once( plugin_dir_path( __FILE__ ).'otw_labels/otw_pswl_factory_object.labels.php' );
		$otw_pswl_factory_object->init();
...
if( !function_exists( 'otw_pswl_init_admin_menu' ) ){
	...
	function otw_pswl_init_admin_menu(){
		...
		add_submenu_page( 'otw-pswl', __( 'Plugin Options', 'otw_pswl'), __('Plugin Options', 'otw_pswl'), 'manage_options', 'otw-pswl-options', 'otw_pswl_plugin_options' );
	}
}
...
/** calls plugin opionts page
  *
  */
function otw_pswl_plugin_options(){
	require_once( 'otw_pswl_plugin_options.php' );
}
 ...
 * factory messages
 */
if( !function_exists( 'otw_pswl_factory_message' ) ){
	function otw_pswl_factory_message( $params ){
		
		global $otw_pswl_plugin_id;
		
		if( isset( $params['plugin'] ) && $otw_pswl_plugin_id == $params['plugin'] ){
